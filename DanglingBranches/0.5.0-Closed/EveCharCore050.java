...
public class EveCharCore extends AbstractGEFNode {
	...
	// - P R O P E R T I E S
	/**
	 * Pointer to the eveapi character sheet structure that contains most of the data. An access to that sheet
	 * will post an update if the contents are obsolete. The EveCharCore will be converted into a proxy to
	 * delegate call to this instance.
	 */
	...
	private HashMap<Long, Property>						locationRoles				= null;
	...
	public void clearLocationRole(final EveLocation theSelectedLocation) {
	...
	public double getBalance() {
		return characterInfo.getAccountBalance();
	}
	...
	public EveLocation getLocation4Role(final String function) {
	...
	public String getLocationRole(final long targetLocationID, final String defaultValue) {
		if (null == locationRoles) accessLocationRoles();
	...
	public String getRace() {
		return characterInfo.getRace().name();
	}

	public String getShipName() {
		return characterInfo.getShipName();
	}
	...
	public String getShipTypeName() {
		return characterInfo.getShipTypeName();
	}
	...
	public void putAction4Item(final int typeID, final String taskName) {
		...
		if (null == hit) {
			hit = new Property(typeID);
			hit.setOwnerID(getCharacterID());
			hit.setPropertyType(EPropertyTypes.MANUFACTUREACTION);
	...
	public void setLocationRole(final EveLocation theSelectedLocation, final String locationrole) {
		if (null == locationRoles) accessLocationRoles();
		Property hit = locationRoles.get(theSelectedLocation.getID());
		if (null == hit) {
			hit = new Property(theSelectedLocation.getID(), EPropertyTypes.LOCATIONROLE, locationrole);
			hit.setOwnerID(getCharacterID());
	...
	public String toString() {
		...
		buffer.append("name: ").append(characterInfo.getCharacterName()).append(" ");
		buffer.append("]");
		return buffer.toString();
	}

	/**
	 * After the API verifies that that character is still defined within the key it should call this method to
	 * download all the character info and character sheet information and refresh what is already contained on
	 * the EveChar. We can expect a complete and from scratch initialization since we are on the load screen.
	 * Afterwards the call will update the already memory loaded information that should be copied to file store
	 * during Activity swaps.
	 */
	public synchronized void updateCharacterInfo() {
		// Use the eveapi to get the character information and store the response on the proxy slot.
		try {
			CharacterInfoParser parser = CharacterInfoParser.getInstance();
			CharacterInfoResponse response = parser.getResponse(getAuthorization());
			if (null != response) {
		...
		try {
			...
			CharacterSheetResponse response = parser.getResponse(getAuthorization());
			if (null != response) {
	...
	private void accessActionList() {
		...
		//		synchronized (actions4Character) {
		...
		for (Property property : actionList) {
			...
			actions4Character.put(property.getId(), property);
		}
		//		}
	}
	...
	private void accessLocationRoles() {
		...
		try {
			...
			PreparedQuery<Property> preparedQuery = queryBuilder.prepare();
			roleList = propertyDao.query(preparedQuery);
		} catch (java.sql.SQLException sqle) {
			sqle.printStackTrace();
		}
		// Process the returned list and store in the character.
		//		synchronized (locationRoles) {
		locationRoles = new HashMap<Long, Property>();
		for (Property property : roleList) {
			// The numeric values stored the location id while the string locates the assigned role.
			long locationID = Double.valueOf(property.getNumericValue()).longValue();
			locationRoles.put(property.getId(), property);
		}
		//		}
	}
	...
	private ApiAuthorization getAuthorization() {
