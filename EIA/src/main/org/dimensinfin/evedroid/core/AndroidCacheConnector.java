...
public class AndroidCacheConnector implements ICache {
	...
	private class DrawableDownloaderTask extends AsyncTask<String, Void, Drawable> {
		...
		protected void onPostExecute(final Drawable result) {
			...
			if (null != result) if (null != this.targetImage) {
	...
	private class EveItemCache extends Statisticscache {
		...
		@Override
		public String toString() {
			//			StringBuffer buffer = new StringBuffer("NewCacheStorage.EveItemCache [");
	...
	private abstract class Statisticscache {
		...
		public String toString() {
			...
			buffer.append("access=").append(this.access).append(" hits=").append(this.hit).append(" miss=").append(this.miss);
			buffer.append(" faults=").append(this.fault);
			if (this.fault > 0) buffer.append("\n").append("Fault Refs [").append(this.itemFaults).append("]");
			buffer.append(" ]");
	...
	//	private final HashMap<String, CacheEntry>			_pendingDrawableDownloads	= new HashMap<String, CacheEntry>();
	...
	public synchronized void addMarketDataRequest(final long localizer) {
		//	Log.i("AndroidCacheConnector", ">> AndroidCacheConnector.addMarketDataRequest");
		...
		int priority = 1;
		if (category.equalsIgnoreCase("Asteroid")) priority = 6;
		if (category.equalsIgnoreCase("Material")) priority = 5;
		if (category.equalsIgnoreCase("Module")) priority = 8;
		if (group.equalsIgnoreCase("Datacores")) priority = 3;
		final PendingRequestEntry request = new PendingRequestEntry(localizer);
		request.setPriority(priority);
	...
	public synchronized Drawable getCacheDrawable(final String urlString, final ImageView target) {
		...
		if (null == hit) synchronized (this) {
			hit = getCache().getByURL(urlString);
			if (null == hit) try {
				//						hit = readDrawableFromDisk(urlString);
				//						if (null == hit) {
				// No luck. We have to download the data from the network.
	...
	public synchronized Vector<PendingRequestEntry> getPendingRequests() {
		if (null == this._pendingRequests) this._pendingRequests = new HashMap<PendingRequestEntry, Integer>();
		...
		for (final PendingRequestEntry entry : this._pendingRequests.keySet())
			if (entry.state != ERequestState.COMPLETED) openRequests.put(entry, this._pendingRequests.get(entry));
	...
	//	/**
	//	 * Searches inside the memory data structures for an already available <code>EveItem</code> with a matching
	//	 * ID. If the object is found we return the single reference to the persistent database information.<br>
	//	 * If the reference is not found we go to the database layer to search for that object and then we keep a
	//	 * copy on memory.<br>
	//	 * Multiple access will require synchronization. The synch has to be performed to the item level to allow
	//	 * other threads searching for different items to continue undisturbed. Double locking will be implemented
	//	 * to avoid waiting threads to go again to database once the lock is removed.<br>
	//	 * The locking will be set on the database access activity. Read operations or update changes will not need
	//	 * to be synchronized.<br>
	//	 * There is a coupled method entry to search by Name instead of ID. The cache data structures keep track of
	//	 * pointers from ID and Name to the same Item.
	//	 * 
	//	 * @param typeID
	//	 *          identifier of the item being searched.
	//	 * @return the persistent <code>EveItem</code> object with the id requested from the
	...
	//	/**
	//	 * This method does the same of the coupled <code>searchItembuID</code> but instead searching for the
	//	 * <code>EveItem</code> by it's ID it will perform the search by the Item Name.
	//	 * 
	//	 * @param name
	//	 * @return
	//	 */
	...
	private void decrementMarketCounter() {
		...
		if (EVEDroidApp.marketCounter < 0) EVEDroidApp.marketCounter = 0;
	...
	private DrawableCache getCache() {
		if (null == this._cacheDrawables) this._cacheDrawables = new DrawableCache();
...
//
//	/**
//	 * Updates the real cache entry state from the elapsed time from the last update end the refresh period set
//	 * to this element class. Also detects empty data that requires a prioritized update.
//	 * 
//	 * @return
//	 */
...
//			boolean needsUpdate = AppConnector.checkExpiration(timestamp, ModelWideConstants.MINUTES60);
