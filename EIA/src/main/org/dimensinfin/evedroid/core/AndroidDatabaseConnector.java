...
public class AndroidDatabaseConnector implements IDatabaseConnector {
	...
	private static final String								REFINING_ASTEROID					= "SELECT itm.materialTypeID AS materialTypeID, itm.quantity AS qty"
																																					...
																																					+ " WHERE itm.typeID = ?"
																																					+ " AND it.typeID = itm.materialTypeID"
																																					+ " AND ito.typeID = itm.typeID"
																																					+ " ORDER BY itm.materialTypeID";
	...
	public Dao<Asset, String> getAssetDAO() throws java.sql.SQLException {
		if (null == appDatabaseHelper) openDAO();
	...
	public SQLiteDatabase getStaticDatabase() throws SQLException {
		if (null == staticDatabase) openAppDataBase();
	...
	public boolean openCCPDataBase() {
		if (null == ccpDatabase) {
			...
			final String path = Environment.getExternalStorageDirectory() + "/" + database;
			try {
				//				InputStream dbStream = _context.getResources().openRawResource(R.raw.eve);
				if (AppConnector.sdcardAvailable())
	...
	public int queryBlueprintDependencies(final int bpitemID) {
		try {
			...
			final Cursor cursor = ccpDatabase.rawQuery(
					"SELECT parentBlueprintTypeID FROM invBlueprintTypes BT WHERE blueprintTypeID = ?", new String[] { Integer
							.valueOf(bpitemID).toString() });
			if (null != cursor) {
	...
	public synchronized void replaceAssets(final long characterID) {
		...
		try {
			...
			synchronized (database) {
				...
				Log.i(
						"",
						"-- replaceAssets rows deleted ASSETS [OWNERID = " + characterID + "] - "
	...
	public synchronized void replaceBlueprints(final long characterID) {
		...
		try {
			...
			synchronized (database) {
				...
				Log.i(
						"",
						"-- replaceBlueprints rows deleted BLUEPRINTS [OWNERID = " + characterID + "] - "
	...
	public synchronized void replaceJobs(final long characterID) {
		...
		try {
			...
			synchronized (database) {
				...
				Log.i(
						"",
						"-- replaceJobs rows deleted JOBS [OWNERID = " + characterID + "] - "
					...
					if (cursor.moveToFirst()) {
						...
						if (tech.equalsIgnoreCase("NOTECH")) {
							...
							if (hit.getName().contains(" II Blueprint")) {
								...
								if (hit.getName().contains(" II Blueprint")) hit.setTech(ModelWideConstants.eveglobal.TechII);
								if (hit.getName().contains(" III Blueprint")) hit.setTech(ModelWideConstants.eveglobal.TechIII);
							}
						} else
							hit.setTech(tech);
					}
	...
	public int searchJobExecutionTime(final int typeID, final int activityID) {
		...
		try {
			if (null == ccpDatabase) ccpDatabase = getCCPDatabase();
			final Cursor cursor = ccpDatabase.rawQuery(JOB_COMPLETION_TIME, new String[] {
					Integer.valueOf(typeID).toString(), Integer.valueOf(activityID).toString() });
			if (null != cursor) {
				while (cursor.moveToNext()) {
	...
	public MarketDataSet searchMarketData(final int itemID, final EMarketSide side) {
		...
		if (side == EMarketSide.BUYER) cache = buyMarketDataCache;
		...
		if (null == entry) {
			...
			if (null == entry) {
				...
				if (true) EVEDroidApp.getTheCacheConnector().addMarketDataRequest(itemID);
			}
