...
public class T1ManufactureProcess extends AbstractManufactureProcess implements IJobProcess {
	...
	public ArrayList<Resource> getLOM() {
		if (null == lom) lom = adjustRequired(AppConnector.getDBConnector().searchListOfMaterials(bpid));
		if (lom.size() == 0) lom = adjustRequired(AppConnector.getDBConnector().searchListOfMaterials(bpid));
		return lom;
	}

	private ArrayList<Resource> adjustRequired(ArrayList<Resource> listofmaterials) {
		for (Resource resource : listofmaterials) {
			// Calculate the real amount of the resource depending on the ME of the blueprint.
			double materialModifier = (100.0 - blueprint.getMaterialEfficiency()) / 100;
			int adjustedQty = Double.valueOf(
					Math.max(1, Math.ceil(Math.round(1.0 * resource.getQuantity() * materialModifier)))).intValue();
	...
	private double calculateCost() {
		...
		for (Resource resource : getLOM()) {
			...
			double resourcePrice = resource.item.getLowestSellerPrice().getPrice();
