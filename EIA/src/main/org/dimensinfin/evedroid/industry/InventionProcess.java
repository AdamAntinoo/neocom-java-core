...
public class InventionProcess extends AbstractManufactureProcess implements IJobProcess {
	...
	public ArrayList<Action> generateActions4Blueprint() {
		...
		// To avoid changing the current cached assets, for this destructive action get a new Manager.
	...
	public double getJobCost() {
		if (cost < 0.0) cost = calculateInventionCost();
		return cost;
	}

	public ArrayList<Resource> getLOM() {
		if (null == lom) lom = AppConnector.getDBConnector().searchListOfDatacores(bpid);
		if (lom.size() == 0) lom = AppConnector.getDBConnector().searchListOfDatacores(bpid);
		return lom;
	}
		...
		if (!blueprint.isPrototype()) {
			...
			if (maxRuns < -1) {
				...
				for (Resource resource : resourceList) {
					...
					if (range < maxRuns) maxRuns = range;
				}
			}
		} else
			maxRuns = 0;
	...
	public void setBlueprint(final Blueprint blueprint) {
		...
		if (!blueprint.isPrototype()) calculateMaxRuns();
	}
	...
	private double calculateInventionCost() {
		...
		for (Resource resource : getLOM()) {
			...
			if (resource.getItem().getGroupName().equalsIgnoreCase("Data Interfaces")) continue;
			...
			resource.setStackSize(1);
			// Calculate the cost.
			MarketDataSet data = AppConnector.getDBConnector()
					.searchMarketData(resource.item.getItemID(), EMarketSide.SELLER);
