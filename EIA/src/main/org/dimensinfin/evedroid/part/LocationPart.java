...
public abstract class LocationPart extends EveAbstractPart implements INamedPart {
	...
	public ArrayList accessLocationFunction() {
		return getPilot().getLocationRoles(getCastedModel().getID(), "-NO-FUNCTION-");
	}
	...
	public String get_locationSystem() {
		return getCastedModel().getSystem();
	}

	public EveLocation getCastedModel() {
		return (EveLocation) getModel();
	}

	@Override
	public long getModelID() {
		return ((EveLocation) getModel()).getID();
	}
	...
	public int searchStationType() {
		return AppConnector.getDBConnector().searchStationType(getCastedModel().getStationID());
	}
	...
	protected void checkAssetStacking(/* final LocationPart target, */final AssetPart apart) {
		...
		HashMap<Integer, AssetPart> container = stackList.get(getCastedModel().getID());
