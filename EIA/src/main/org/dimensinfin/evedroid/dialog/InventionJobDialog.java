...
public class InventionJobDialog extends DialogFragment {
	...
	public Dialog onCreateDialog(final Bundle savedInstanceState) {
		...
		if (null == this._blueprint) return null;
		initializeViews();
		setupContents();
		...
		final int runs = 6;
		//		final double intermediate = (1.0 * this._blueprint.getMaxRuns()) / (1.0 * runs);
		...
		this._runsCount.addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(final Editable newValue) {
				...
				if (currentruns > InventionJobDialog.this._maxruncount)
					InventionJobDialog.this._runsCount.setText(Integer.valueOf(InventionJobDialog.this._maxruncount).toString());
				else {
					//					final double intermediate = (1.0 * currentruns) / (1.0 * runs);
					//					InventionJobDialog.this._jobs = Math.min(Double.valueOf(Math.ceil(intermediate)).intValue(), bpccount);
					...
					InventionJobDialog.this._jobDuration.setText(EveAbstractPart.generateTimeString(jobDuration
							* Math.min(currentruns, runs)));
				}
			}
						...
						public void onClick(final DialogInterface dialog, final int id) {
							try {
								if (null != InventionJobDialog.this._runsCount) {
									InventionJobDialog.this._runs = Integer.parseInt(InventionJobDialog.this._runsCount.getEditableText()
											.toString());
									Toast.makeText(getActivity(), "Selected Runs: " + InventionJobDialog.this._runs, Toast.LENGTH_LONG);
	...
	public void setBlueprint(final Resource bp) {
		//REFACTOR Implemente the assignment of the resource as a blueprint. What is the received resource?.
		this._blueprint = new Blueprint(bp.getTypeID());
	}

	public void setDialogCallback(final ADialogCallback callback) {
		if (null != callback) this._dialogCallback = callback;
	}
	...
	public void setupContents() {
		...
		this._blueprintMETE.setText(this._blueprint.getMaterialEfficiency() + " / " + this._blueprint.getTimeEfficiency());
	}
