...
public class DirectorsBoardActivity extends AbstractContextActivity {
	...
	protected void onCreate(final Bundle savedInstanceState) {
		...
		try {
			...
			logger.severe("R> Runtime Exception on DirectorsBoardActivity.onCreate." + rtex.getMessage());
			rtex.printStackTrace();
			stopActivity(new RuntimeException(
					"R> Runtime Exception on DirectorsBoardActivity.onCreate." + rtex.getMessage()));
		}
		logger.info("<< DirectorsBoardActivity.onCreate");
	}

	/**
	 * Save the store to their persistent file before releasing the control to
	 * another activity that will then be able to make use of that data
	 * structures.
	 */
	@Override
	...
	/**
	 * When the Activity is activated and accessible to the user this event is
	 * signaled to report that we are back. On the list of managers we have to
	 * check manager by manager if they should activate and if they are or not
	 * visible.<br>
	 * This is done with a manager factory that will generate the view items for
	 * the managers if they should be present.
	 */
	@SuppressLint("NewApi")
		...
		try {
			...
			for (final EDirectorCode directorCode : directors) {
				...
				case INDUSTRYDIRECTOR:
					...
					break;
				case JOBDIRECTOR:
					final IDirector jdirector = new JobDirectorActivity();
	...
	protected void onSaveInstanceState(final Bundle savedInstanceState) {
		...
		savedInstanceState.putLong(AppWideConstants.extras.EXTRA_EVECHARACTERID,
				this._store.getPilot().getCharacterID());
		// _store.save();
		Log.i("DirectorsBoardActivity", "<< DirectorsBoardActivity.onSaveInstanceState"); //$NON-NLS-1$
	}

	/**
	 * Clear the state of all the Directors to a dimmed and inactive state. It
	 * will be recalculated while on the onResume phase.
	 */
	@Override
	@SuppressLint("NewApi")
	@TargetApi(Build.VERSION_CODES.ICE_CREAM_SANDWICH_MR1)
	...
	private void addExpandableFragment(final String label, final int fragmentIdentifier) {
		final FragmentManager manager = getFragmentManager();
		if (null != manager)
			if (null == manager.findFragmentByTag(label)) {
	...
	/**
	 * Adds a new fragment to the fragment manager in case that fragment is not
	 * already registered. Fragment registration uses the fragment identifier
	 * converted to an string as the unique locator. If the fragment is not
	 * registered the methods creates a new instance and initializes its
	 * structures.
	 * 
	 * @param label
