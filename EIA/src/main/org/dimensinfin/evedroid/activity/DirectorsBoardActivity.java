...
public class DirectorsBoardActivity extends AbstractContextActivity {
	...
	private static final EDirectorCode[]	activeDirectors			= { EDirectorCode.ASSETDIRECTOR,
			EDirectorCode.SHIPDIRECTOR, EDirectorCode.INDUSTRYDIRECTOR, EDirectorCode.JOBDIRECTOR,
			EDirectorCode.MARKETDIRECTOR													};
	...
	protected void onCreate(final Bundle savedInstanceState) {
		...
		try {
			// Add the fragments to the fragment container if they are not already there.
			...
			logger.severe("R> Runtime Exception on DirectorsBoardActivity.onCreate." + rtex.getMessage());
			rtex.printStackTrace();
			stopActivity(new RuntimeException("R> Runtime Exception on DirectorsBoardActivity.onCreate." + rtex.getMessage()));
		}
		logger.info("<< DirectorsBoardActivity.onCreate");
	}
	...
	protected void onResume() {
		...
		try {
			...
			//			getPilot().clean();
			// Before activating the directors make sure the user has declared the MANUFACTURE location.
			//			final EveLocation location = _store.getPilot().getFunctionLocation("MANUFACTURE");
			...
			for (final EDirectorCode directorCode : directors) {
						...
						if (adirector.checkActivation(this._store.getPilot())) {
							...
							activator.setOnClickListener(new View.OnClickListener() {
								...
								public void onClick(final View view) {
									...
									intent.putExtra(AppWideConstants.extras.EXTRA_EVECHARACTERID, DirectorsBoardActivity.this._store
				...
				//							logger.info("-- DirectorsBoardActivity.onResume - activated " + directorCode);
				//							activator = (ImageView) findViewById(R.id.miningDirectorIcon);
				//							activator.setImageDrawable(getDrawable(R.drawable.miningdirector));
				//							activator.setClickable(true);
				//							activator.setOnClickListener(new View.OnClickListener() {
				//								public void onClick(final View view) {
				//									// Activate the manager.
				//									final Intent intent = new Intent(parentActivity, mdirector.getClass());
				//									// Send the pilot id and transfer it to the next Activity
				//									intent.putExtra(AppWideConstants.extras.EXTRA_EVECHARACTERID, parentActivity.getPilot()
				//											.getCharacterID());
				//									startActivity(intent);
				//								}
				//							});
				...
				//								public void onClick(final View view) {
				//									// Activate the manager.
				//									final Intent intent = new Intent(parentActivity, tdirector.getClass());
				//									// Send the pilot id and transfer it to the next Activity
				//									intent.putExtra(AppWideConstants.extras.EXTRA_EVECHARACTERID, parentActivity.getPilot()
				//											.getCharacterID());
				//									startActivity(intent);
				...
				//							public void onClick(final View view) {
				//								// Activate the manager.
				//								final Intent intent = new Intent(parentActivity, FittingActivity.class);
				//								// Send the pilot id and transfer it to the next Activity
			...
			rtex.printStackTrace();
			stopActivity(new RuntimeException("R> Runtime Exception on DirectorsBoardActivity.onResume." + rtex.getMessage()));
		}
		logger.info("<< DirectorsBoardActivity.onResume");
	}
