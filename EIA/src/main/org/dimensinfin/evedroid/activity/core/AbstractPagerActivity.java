...
public abstract class AbstractPagerActivity extends Activity {
			...
			this._indicator.setOnPageChangeListener(new ViewPager.OnPageChangeListener() {
				...
				public void onPageSelected(final int position) {
					...
					if ("" == AbstractPagerActivity.this._pageAdapter.getSubTitle(position))
						...
						AbstractPagerActivity.this._actionBar.setSubtitle(AbstractPagerActivity.this._pageAdapter
	...
	protected void addPage(final AbstractPagerFragment newFrag, final int position) {
		...
		final AbstractPagerFragment frag = (AbstractPagerFragment) getFragmentManager().findFragmentByTag(
				this._pageAdapter.getFragmentId(position));
		if (null == frag)
		...
		if (_pageAdapter.getCount() > 1) activateIndicator();
		Log.i("NEOCOM", "<< AbstractPagerActivity.addPage"); //$NON-NLS-1$
	}

	protected void disableIndicator() {
		if (null != this._indicator) this._indicator.setVisibility(View.GONE);
	}
