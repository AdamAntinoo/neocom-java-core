...
public abstract class EVEPagerActivity extends DefaultNewPagerActivity implements IActivityMessageBus {
	...
	// ..........................................................................
	/**
	 * This is the request to get the part hierarchy for a body panel. The
	 * parameter determines the variant to be used. This is finally implemented
	 * by the DataSourceFragment.
	 */
	...
	/**
	 * This is the request to get the part hierarchy to set into the header
	 * panel. The parameter determines the variant to be used. This is finally
	 * implemented by the DataSourceFragment. The Fragment will have to handle
	 * that list because the header panel is not managed by an adapter but it is
	 * a layout container.
	 */
	...
	/**
	 * Event messages may come from the parts through the data source proxy.
	 * Those messages should be relayed to the data source. But the messages
	 * from the datasource should instead be passed back to the fragments.
	 */
	...
	protected TitledFragment createBodyFragment(final int fragmentID, final int pageID) {
		...
		try {
			...
			NewPagerFragment frag = (NewPagerFragment) getFragmentManager()
					.findFragmentByTag(this._pageAdapter.getFragmentId(pageID));
			if (null == frag) {
	...
	private ArrayList<NewPagerFragment> getFragments() {
		if (null == this._fragments) {
			this._fragments = new ArrayList<NewPagerFragment>();
		}
