...
//	}
	...
	public PilotV2 setProperties( final List<Property> properties ) {
		...
		for (Property prop : properties) {
			if (prop.getPropertyType() == PropertyTypes.MANUFACTUREACTION)
				actions4Pilot.put( Double.valueOf( prop.getNumericValue() ).intValue(), prop );
			if (prop.getPropertyType() == PropertyTypes.LOCATIONROLE)
	...
	public Property addLocationRole( final EsiLocation theSelectedLocation, final String locationrole ) {
		Property hit = new Property.Builder()
				.withPropertyType( PropertyTypes.LOCATIONROLE )
				.withOwnerId( this.getPilotId() )
				.withTargetId( theSelectedLocation.getId() )
				.build();
...
//	}
	...
	public void addAction4Item( final int typeId, final String taskName ) {
		...
		if (null == hit) {
			hit = new Property.Builder()
					.withPropertyType( PropertyTypes.MANUFACTUREACTION )
					.withOwnerId( this.getPilotId() )
					.build();
