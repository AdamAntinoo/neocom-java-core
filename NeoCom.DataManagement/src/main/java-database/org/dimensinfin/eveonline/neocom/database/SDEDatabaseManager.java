...
public abstract class SDEDatabaseManager {
	...
	public List<Schematics> searchSchematics4Output( final int targetId ) {
		...
		try {
			...
			while (cursor.moveToNext()) {
				// TODO - This code should be removed asap becuase it is already unused but can be referenced on the planetary processor.
				scheList.add(new Schematics.Builder().build());
			}
			cursor.close();
		} catch (final Exception ex) {
