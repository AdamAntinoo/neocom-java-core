...
public class MarketDataService implements Runnable {
	...
	public static Vector<MarketDataSet> marketDataServiceEntryPoint(final int localizer) {
		...
		final EveItem item = NeoComAppConnector.getSingleton().getCCPDBConnector().searchItembyID(localizer);
	...
	private static EveLocation generateLocation(String hubName) {
		...
		return NeoComAppConnector.getSingleton().getCCPDBConnector().searchLocationBySystem(hubSystem);
	}
