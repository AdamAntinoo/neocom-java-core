...
public class CryosBlueTheme implements ITheme {
	...
	public Typeface getThemeTextFont() {
		return CryosBlueTheme.daysFace;
	}
	...
	public Drawable getThemeTransparent(final EThemeTransparency level) {
		...
		if (level == EThemeTransparency.VERYLOW) {
			draw = this.getResourceManager().getDrawable(R.drawable.redtraslucent20);
		}
		if (level == EThemeTransparency.LOW) {
			draw = this.getResourceManager().getDrawable(R.drawable.redtraslucent40);
		}
		if (level == EThemeTransparency.MEDIUM) {
			draw = this.getResourceManager().getDrawable(R.drawable.redtraslucent80);
		}
		if (level == EThemeTransparency.HIGH) {
			draw = this.getResourceManager().getDrawable(R.drawable.redtraslucentb0);
		}
		return draw;
	}
