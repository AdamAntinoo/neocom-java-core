...
public class NeoComSBDBHelper {
	...
	public static class DaoBuilder<D> {
		private static final Map<Class, Dao> daoAvailable = new HashMap();
		private static ConnectionSource connection;

		private DaoBuilder() {
		}

		public Dao getGenericDao( final Class classType ) throws SQLException {
			Dao hit = daoAvailable.get(classType);
			if (null == hit) {
				hit = DaoManager.createDao(connection, classType);
				daoAvailable.put(classType, hit);
			}
			return hit;
		}
	}
